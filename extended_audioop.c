...
#ifdef DEBUG
void printBin(unsigned char *cp, int i)
{
	unsigned char c = 0;
	int byte,bit;
	signed char * p = (signed char *)cp+i;
...
void binPrintInt(int i)
{
	int bit;
//	printf("\n\nSize of jaj_24Bits: %lu\n\n", sizeof(jaj_24Bits));
	printf("\nInt = %x (%d)\n\t",i,i);
	for (bit = 1; bit<33; bit++) {
...
audioop_mul(PyObject *self, PyObject *args)
{
    ...
    else if ( size == 3 ) maxval = (double) 0x7fffff;
    ...
    for ( i=0; i < len; i += size ) {
        ...
        else if ( size == 3 ) *LONGP(ncp, i) = (Py_Int32)val;
...
audioop_tomono(PyObject *self, PyObject *args)
{
    ...
    for ( i=0; i < len; i += size*2 ) {
        ...
        else if ( size == 3 ) *LONGP(ncp, i/2)= (Py_Int32)val1;
...
audioop_tostereo(PyObject *self, PyObject *args)
{
    ...
    for ( i=0; i < len; i += size ) {
        ...
        else if ( size == 3 ) *LONGP(ncp, i*2+3) = (Py_Int32)val2;
...
audioop_add(PyObject *self, PyObject *args)
{
    ...
    else if ( size == 3 ) maxval = 0x7fffff;
    ...
    for ( i=0; i < len1; i += size ) {
        if ( size == 1 )      val1 = (int)*CHARP(cp1, i);
            ...
            val1 = (int)*LONGP(cp1, i) & 0x00ffffff;
        ...
        if ( size == 1 )      val2 = (int)*CHARP(cp2, i);
            ...
            val2 = (int)*LONGP(cp2, i) & 0x00ffffff;
        ...
        else if ( size == 3 ) *LONGP(ncp, i) = (Py_Int32)newval;
...
audioop_bias(PyObject *self, PyObject *args)
{
    ...
    for ( i=0; i < len; i += size ) {
        ...
        else if ( size == 3 ) *LONGP(ncp, i) = (Py_Int32)(val+bias);
...
audioop_reverse(PyObject *self, PyObject *args)
{
    ...
    for ( i=0; i < len; i += size ) {
        ...
        if ( size == 1 )      *CHARP(ncp, j) = (signed char)(val >> 8);
				...
				else if ( size == 3 ) *LONGP(ncp, j) = (Py_Int32)(val<<8);
...
audioop_lin2lin(PyObject *self, PyObject *args)
{
    ...
    for ( i=0, j=0; i < len; i += size, j += size2 ) {
        ...
        {
            int dbg = i/size;
            if (dbg > 100000 && dbg < 100010) {
                 printf("INPUT INT = %d -- OUTPUT SHORT = %hd\n",val,(short)val);
                 printf("INPUT INT = %d -- INT = %d\n",val,(Py_Int32)(val) << 8);
				    }
        }

        if ( size2 == 1 )  *CHARP(ncp, j) = (signed char)(val >> 8);
        else if ( size2 == 2 ) *SHORTP(ncp, j) = (short)(val);
        else if ( size2 == 3 ) *LONGP(ncp, j) = (Py_Int32)(val)<<8;
        ...
        while (d < 0) {
            ...
            for (chan = 0; chan < nchannels; chan++) {
                ...
                if (size == 1)
                    ...
                    cur_i[chan] = ((int)*LONGP(cp, 0)) & 0x00ffffff;
                    cur_i[chan] |= (cur_i[chan] & 0x00800000) ? 0xff000000 : 0;
                    cur_i[chan] = cur_i[chan] >> 8;
                } else if (size == 4)
                    cur_i[chan] = ((int)*LONGP(cp, 0)) >> 16;

#ifdef DEBUG
                if (frame > 100000 && frame < 100010) {
                    print_debug = 1;
                    //if (size == 3 && cur_i[chan] & 0x00800000) cur_i[chan] = cur_i[chan] | 0xff000000;
                   printf("Output: %d",cur_i[chan]);
                   binPrintInt(cur_i[chan]);
                  //dbg = *(jaj_24Bits*)LONGP(cp, 0);
                  //binPrintInt((*(int*)(void*)&dbg) & 0x00ffffff);
                }
                frame++;
#endif
                cp += size;
        ...
        while (d >= 0) {
            for (chan = 0; chan < nchannels; chan++) {
                ...
                if (size == 1)
                    ...
                    *LONGP(ncp, 0) = ((Py_Int32)(cur_o<<8));
                else if (size == 4)
                    *LONGP(ncp, 0) = (Py_Int32)(cur_o<<16);

#ifdef DEBUG
                if (print_debug != 0) {
                    printf("\nInput: %p = %x (%d) -- cur_o = %x", ncp, *(int*)ncp, *(int*)ncp, cur_o);
                    binPrintInt(*(int*)ncp);
                }
#endif
                ncp += size;
...
audioop_lin2ulaw(PyObject *self, PyObject *args)
{
    ...
    for ( i=0; i < len; i += size ) {
        if ( size == 1 )      val = ((int)*CHARP(cp, i)) << 8;
            ...
            val = ((int)*LONGP(cp, 0)) & 0x00ffffff;
            val |= (val & 0x00800000) ? 0xff000000 : 0;
						val = val >> 8;
        } else if ( size == 4 ) val = ((int)*LONGP(cp, i)) >> 16;
...
audioop_ulaw2lin(PyObject *self, PyObject *args)
{
    ...
    for ( i=0; i < len*size; i += size ) {
        ...
        else if ( size == 3 ) *LONGP(ncp, i) = (Py_Int32)(val<<8);
...
audioop_lin2alaw(PyObject *self, PyObject *args)
{
    ...
    for ( i=0; i < len; i += size ) {
        ...
        else if ( size == 3 ) val = ((int)*LONGP(cp, i)) >> 8;
        else if ( size == 4 ) val = ((int)*LONGP(cp, i)) >> 16;
